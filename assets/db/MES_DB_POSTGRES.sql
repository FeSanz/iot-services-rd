...
CREATE TABLE MES_DASHBOARDS (
    ...
    UPDATED_DATE TIMESTAMPTZ,
    UPDATED_BY TEXT,
    CONSTRAINT FK_DASHBOARDS_USER FOREIGN KEY (USER_ID) REFERENCES MES_USERS(USER_ID)
);
...
-- Crear tablas con dependencias de tercer nivel

-- Tabla de Relación Usuarios-Máquinas
CREATE TABLE MES_USERS_MACHINES (
    USER_MACHINE_ID SERIAL PRIMARY KEY,
    USER_ID INTEGER,
    MACHINE_ID INTEGER,
    CONSTRAINT FK_USERS_MACHINES_USER FOREIGN KEY (USER_ID) REFERENCES MES_USERS(USER_ID),
    CONSTRAINT FK_USERS_MACHINES_MACHINE FOREIGN KEY (MACHINE_ID) REFERENCES MES_MACHINES(MACHINE_ID)
);
