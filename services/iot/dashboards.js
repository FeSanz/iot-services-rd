...
router.get('/dashboards/:userId', async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT * FROM mes_dashboards WHERE user_id = $1 ORDER BY dashboard_id ASC', [userId]
        );
...
router.post('/dashboards', async (req, res) => {
    const { user_id, name, color, parameters, created_by, updated_by } = req.body;

    try {
        const result = await pool.query(
            `INSERT INTO mes_dashboards ( user_id, name, color, parameters, created_date, created_by, updated_date, updated_by ) VALUES ( $1, $2, $3, $4::jsonb, CURRENT_TIMESTAMP, $5, CURRENT_TIMESTAMP, $6 )
            RETURNING *`,
            [user_id, name, color, parameters, created_by, updated_by]
        );
...
router.put('/dashboards/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query(`UPDATE mes_dashboards SET user_id = $1, name = $2, color = $3, parameters = $4::jsonb, updated_date = CURRENT_TIMESTAMP, updated_by = $5 WHERE dashboard_id = $6 RETURNING *`,
            [user_id, name, color, parameters, updated_by, dashboardId]
        );
